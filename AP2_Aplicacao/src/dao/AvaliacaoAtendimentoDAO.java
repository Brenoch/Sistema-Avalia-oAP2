...
public class AvaliacaoAtendimentoDAO implements BaseDAO {
                ...
                if (affectedRows > 0) {
                    try (ResultSet generatedKeys = stmt.getGeneratedKeys()) {
                        if (generatedKeys.next()) {
                            ...
                            System.out.println("Avaliação de atendimento foi salva com sucesso no banco de dados (ID: " + avaliacao.getIdAvaliacao() + ")");
                        } else {
                            System.err.println("Não foi possível obter o ID gerado para a avaliação de atendimento.");
                        }
                    }
                } else {
                    System.err.println("Nenhuma linha modificada ao salvar avaliação de atendimento.");
                }

            } catch (SQLException e) {
                System.err.println("Não foi possível salvar avaliação de atendimento no banco de dados: " + e.getMessage());
                e.printStackTrace();
            }
        } else {
            System.out.println("Objeto não é uma instância de AvaliacaoAtendimento. Não foi salvo no banco de dados.");
        }
    }
    ...
    public void excluir(int id) {
        ...
        try (Connection conn = ConnectionFactory.getConnection();
            ...
            System.err.println("Não foi possível excluir Avaliação: " + e.getMessage());
            e.printStackTrace();
        }
    }
}