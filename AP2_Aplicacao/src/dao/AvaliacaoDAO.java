        ...
        if (obj instanceof AvaliacaoAmbiente) {
            ...
            try (Connection conn = ConnectionFactory.getConnection();
                ...
                if (avaliacao.getRestaurante() == null || avaliacao.getRestaurante().getIdrestaurante() == 0) {
                    throw new SQLException("O restaurante associado à avaliação é nulo ou não tem ID válido. Salve o Restaurante primeiro.");
                }
                if (avaliacao.getCliente() == null || avaliacao.getCliente().getIdcliente() == 0) {
                    throw new SQLException("O cliente associado à avaliação é nulo ou não tem ID válido. Salve o Cliente primeiro.");
                }
                ...
                if (affectedRows > 0) {
                    ...
                    System.err.println("Nenhuma modificada afetada ao salvar a Avaliação de Ambiente.");
                }

            } catch (SQLException e) {
                System.err.println("Não foi possível ao salvar: " + e.getMessage());
                e.printStackTrace();
            }
        } else {
    ...
    public void atualizar(Object obj) {
        if (obj instanceof AvaliacaoAmbiente) {
            ...
            try (Connection conn = ConnectionFactory.getConnection();
                ...
                stmt.setFloat(1, avaliacao.getNotaAmbiente());
                if (avaliacao.getRestaurante() == null || avaliacao.getRestaurante().getIdrestaurante() == 0) {
                    throw new SQLException("O restaurante associado à avaliação é nulo ou não tem ID válido.");
                }
                if (avaliacao.getCliente() == null || avaliacao.getCliente().getIdcliente() == 0) {
                    throw new SQLException("O cliente associado à avaliação é nulo ou não tem ID válido.");
                }
                stmt.setInt(2, avaliacao.getRestaurante().getIdrestaurante());
                stmt.setInt(3, avaliacao.getCliente().getIdcliente());
            ...
            System.out.println("O objeto não é uma instância de AvaliacaoAmbiente. Não atualizado.");
        }
    }
